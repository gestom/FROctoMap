...
void callback(const boost::shared_ptr<const sensor_msgs::PointCloud2>& msg)
{
  ...
  listener->waitForTransform(msg->header.frame_id.c_str(), "/", ros::Time(), ros::Duration(3.0));
...
int main(int argc, char **argv)
{
  ...
  private_node_handle_.param("pc2_input", pc_sub_topic, string("/local_metric_map/merged_point_cloud"));
  message_filters::Subscriber<sensor_msgs::PointCloud2> sub;
  sub.subscribe(private_node_handle_, pc_sub_topic.c_str(), 10);
  tf_filter = new tf::MessageFilter<sensor_msgs::PointCloud2>(sub, *listener, "/head_xtion_rgb_optical_frame", 10);
  tf_filter->registerCallback(boost::bind(&callback, _1));
  private_node_handle_.param("pc2_output", pc_pub_topic, string("/froctomap/localpoints"));
